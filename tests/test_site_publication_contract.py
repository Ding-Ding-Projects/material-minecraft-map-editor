...
class SitePublicationContractTests(unittest.TestCase):
    ...
    def test_verified_manifest_requires_immutable_release_assets(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            manifest.update({"verified": True, "releaseTag": "0.10.99", "commit": "a" * 40})
            manifest["assets"] = {
                name: {
                    "name": "Amulet-0.10.99-full.nupkg" if name == "full.nupkg" else name,
                    "url": f"https://downloads.example.test/releases/download/0.10.99/Amulet-0.10.99-full.nupkg" if name == "full.nupkg" else f"https://downloads.example.test/releases/download/0.10.99/{name}",
                    "sha256": "b" * 64,
                }
