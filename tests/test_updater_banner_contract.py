...
def test_update_banner_has_persistent_actions_and_bounded_refresh():
    ...
    assert "validate_release_notes_url" in SOURCE
    assert "wx.LaunchDefaultBrowser(release_notes_url)" in SOURCE
    assert "release_notes_url=release_notes_url" in SOURCE
...
def test_update_states_are_rendered_without_modal_dialogs():
    ...
    update_function = SOURCE[
        SOURCE.index("def _show_update_state") : SOURCE.index(
