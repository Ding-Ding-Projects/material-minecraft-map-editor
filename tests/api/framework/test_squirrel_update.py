...
_MODULE_PATH = Path(__file__).parents[3] / "amulet_map_editor/api/framework/squirrel_update.py"
...
def test_feed_requires_https_and_no_credentials():
    assert validate_feed_url("https://updates.example.test/releases/").startswith("https://")
...
def test_check_reports_available_without_wx(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        squirrel_update, "_run_update", lambda *_args, **_kwargs: {"futureReleaseEntry": {"version": "1.2.3"}}
    )
    state = check_for_update("https://updates.example.test/releases/", update_exe=updater)
