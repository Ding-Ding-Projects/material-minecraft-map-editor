...
def test_refresh_applies_only_valid_values():
    ...
    coordinator = ScheduledRefreshCoordinator(lambda *_args, **_kwargs: SourceResult(True, {"theme": "dark"}))
...
def test_refresh_is_non_blocking_on_fetch_or_apply_failure():
    source = ScheduleSource(kind="api", url="https://example.test/feed")
    coordinator = ScheduledRefreshCoordinator(lambda *_args, **_kwargs: SourceResult(False, {}, "offline"))
    result = coordinator.refresh(source, apply=lambda _values: (_ for _ in ()).throw(RuntimeError("apply failed")))
