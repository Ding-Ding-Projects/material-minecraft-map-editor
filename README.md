...
| Squirrel update bridge | Covered by wx-independent tests for canonical explicit-channel discovery across five bounded inventory pages, exact route/status/content validation on every page, official progress/JSON command parsing with bounded stdout and stderr, exact post-stage version proof, immediate-layout updater discovery, a 900-second stage budget, and the guarded process-start-and-wait restart transaction. |
