...
| electron-migration | `electron/main.js`, `electron/preload.js`, `amulet_map_editor/api/sidecar/server.py`, `protocol.py` | `docs/features/electron-migration/README.md` | n/a (no `docs/site/` copy calls the sidecar yet) | **gap:** no bridge — `electron/preload.js` exposes no sidecar method, `docs/site/` never calls one, so nothing a user does in the shell reaches the Python core | n/a for the missing bridge; sidecar itself driven directly by a standalone script for this article | n/a | `scripts/capture_electron_shell.js` → `docs/huishots/electron/electron-shell-home.png` (shell renders the real site; does not exercise the sidecar) | **incomplete** — shell renders, sidecar answers real calls in isolation, the two are not wired together |
