...
| electron-migration | `electron/main.js`, `electron/preload.js`, `electron/sidecar-client.js`, `docs/site/electron-bridge.js`, `amulet_map_editor/api/sidecar/server.py`, `protocol.py`, `methods.py`, `world_methods.py`, `mesh_methods.py` | `docs/features/electron-migration/README.md`, `docs/features/electron-acceptance/README.md` | `docs/site/electron-bridge.js` calls the real `window.mmweDesktop.sidecar.call(...)` bridge for preferences/language/changelog/docs/converter/world | `amulet_map_editor/api/preferences.py` file, round-tripped through the bridge and surviving a real app restart | `tests/test_electron_sidecar_bridge.py` | `scripts/capture_electron_sidecar_roundtrip.js`, `scripts/capture_viewport_shell_with_selection.js`, `scripts/accept_electron_app.js` (11-check acceptance run against the **packaged** executable, not `electron .`) | `scripts/accept_electron_app.js` → `docs/huishots/electron/accept-electron-app-manifest.json`, `accept-selection-with.png`/`accept-selection-without.png` (all 11 checks pass as of the acceptance run that also found and fixed a packaging defect: the sidecar's Python source was never bundled into any prior packaged build, and its `cwd` resolved *inside* `app.asar` once packaged — see the acceptance article) | **incomplete** — the bridge is real and wired end to end (preferences, language, changelog, docs, world, viewport, selection) and now works from the packaged installer; the one remaining gap is `converter.formats` having no matching `converter.convert` sidecar method, so a file conversion still cannot be driven from the Electron shell over IPC (proven directly against `amulet_map_editor.api.converter.core` instead) |
