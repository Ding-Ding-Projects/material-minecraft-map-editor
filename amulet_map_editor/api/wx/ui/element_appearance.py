...
def apply_override(control: wx.Window) -> None:
    ...
    if not override:
        return
    for field, setter in (("background", control.SetBackgroundColour), ("foreground", control.SetForegroundColour)):
...
class ElementAppearanceDialog(wx.Dialog):
    ...
    def __init__(self, parent: wx.Window, control: wx.Window):
        super().__init__(parent, title="Edit appearance", size=wx.Size(500, 460), style=wx.NO_BORDER | wx.RESIZE_BORDER)
        ...
        self.background = wx.TextCtrl(self, value=str(values.get("background", "")), name="Element background HEX")
        self.background.SetHint("Background HEX, for example #6750A4")
        self.foreground = wx.TextCtrl(self, value=str(values.get("foreground", "")), name="Element foreground HEX")
        ...
        self.Bind(wx.EVT_BUTTON, lambda _event: self.EndModal(wx.ID_CANCEL), id=wx.ID_CANCEL)
    ...
    def _save(self, _event) -> None:
        values = self._values()
        if any(value and not re.fullmatch(r"#[0-9a-fA-F]{6}", value) for value in (values["background"], values["foreground"])):
            self.feedback.SetLabel("Use six-digit HEX colours such as #6750A4, or leave a field blank.")
            return
...
__all__ = ["ElementAppearanceDialog", "apply_override", "element_key", "open_element_appearance", "reset_override", "save_override"]
